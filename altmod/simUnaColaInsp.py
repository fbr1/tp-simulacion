...
class SimulacionUnaColaInsp(Simulacion):
    ...
    def partida_pala(self, camion):
        ...
        for i in range(len(self.estado_inspeccion)):
            if self.estado_inspeccion[i] == SimulacionUnaColaInsp.DESOCUPADO:
                desocupado = True
                # Generar el fin de inspeccion
                tiempo = camion.tiempo_de_mantenimiento() + self.reloj_simulacion
                self.lista_de_eventos['fin_de_inspeccion'][i].agregar(tiempo, camion)
                self.estado_inspeccion[i] = SimulacionUnaColaInsp.OCUPADO

                break
        if not desocupado:
            self.cola_inspeccion.agregar(camion)

        # Si hay camiones en la cola
        cola_pala = self.cola_pala

        if cola_pala.cola:

            # Quitar al primer camion de la cola
            camion_cola = cola_pala.cola.pop(0)
            camion_cola.nro_pala = camion.nro_pala

            # Generar la partida (tiempo de carga) del camion que sale de la cola
            tiempo = camion_cola.tiempo_de_carga() + self.reloj_simulacion
            self.lista_de_eventos['partida_pala'][camion_cola.nro_pala].agregar(tiempo, camion_cola)

        else:
            self.estado_pala[camion.nro_pala] = SimulacionUnaColaInsp.DESOCUPADO
    ...
    def fin_de_inspeccion(self, camion):
        ...
        if self.cola_inspeccion.cola:

            camion_cola = self.cola_inspeccion.cola.pop(0)
            camion_cola.nro_pala = camion.nro_pala
            ...
            self.estado_inspeccion[camion.nro_pala] = SimulacionUnaColaInsp.DESOCUPADO
    ...
    def arribo_pala(self, camion):
        ...
        if numero_aleatorio < SimulacionUnaColaInsp.PROB_DESCOMPOSTURA:
            ...
            for i in range(len(self.estado_pala)):
                if self.estado_pala[i] == SimulacionUnaColaInsp.DESOCUPADO:
                    desocupado = True
                    camion.nro_pala = i
                    ...
                    self.estado_pala[camion.nro_pala] = SimulacionUnaColaInsp.OCUPADO
    ...
    def inicializacion(self, duracion_simulacion):
        ...
        super(SimulacionUnaColaInsp, self).inicializacion(duracion_simulacion)

        self.estado_inspeccion = [SimulacionUnaColaInsp.DESOCUPADO, SimulacionUnaColaInsp.DESOCUPADO, SimulacionUnaColaInsp.DESOCUPADO]
        self.cola_pala = Cola('pala')
        self.cola_inspeccion = Cola('inspeccion')
    ...
    def generar_evento_desencadenador(self):
        for i in range(3):
            ...
            self.estado_pala[i] = SimulacionUnaColaInsp.OCUPADO

    def __init__(self):
        super(SimulacionUnaColaInsp, self).__init__()
        self.cola_pala = None
